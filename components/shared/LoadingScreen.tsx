...
const DataStream = ({ side }: { side: "left" | "right" }) => {
  ...
  return (
    <div
      className={`absolute top-0 bottom-0 ${side}-4 w-12 hidden lg:flex flex-col gap-2 font-mono text-[8px] opacity-[0.05] pointer-events-none select-none py-10`}
    >
      {data.map((str, i) => (
        <span key={i}>{str}</span>
      ))}
    ...
    <AnimatePresence>
      {loading && (
        ...
        >
          ...
          <motion.div
            animate={
              exitPhase === "vortex" || exitPhase === "pulse"
                ? { opacity: 0 }
                : { opacity: 0.03 }
            }
          ...
          <motion.div
            animate={exitPhase !== "loading" ? { opacity: 0, scale: 0 } : {}}
          >
                      ...
                      {hasHydrated && (
                        <role.icon
                          className={`w-5 h-5 transition-all duration-500 ${
                            activeStep === i
                              ? "text-primary scale-125 drop-shadow-[0_0_8px_var(--primary)]"
                              : "opacity-20"
                          }`}
                        />
                      )}
          ...
          >
            <div className="absolute bottom-[34px] left-[38px] flex flex-col gap-1 h-24">
              <div className="text-[8px] font-bold text-primary/40 uppercase mb-2">
                // INIT_LOGS:
              </div>
